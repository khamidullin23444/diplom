...
Система использует архитектуру на основе **LangGraph** для координации работы трёх ИИ-агентов:
1. **OCR агент** - распознавание текста с помощью YOLOv8
2. **LLM агент** - исправление ошибок и лексический анализ
3. **Агент транслитерации** - перевод распознанного текста/символов в удобочитаемую латиницу/кириллицу
...
└──────┬───────┘
       ...
       ▼
┌─────────────┐      ┌──────────────┐      ┌────────────────────┐      ┌─────────────┐
│ OCR Agent   │─────▶│ LLM Agent    │─────▶│ Transliteration    │─────▶│  Результаты │
│ (YOLOv8)    │      │ (tweety-tatar│      │ Agent              │      │             │
└─────────────┘      │  -base)      │      └────────────────────┘      └─────────────┘
                     └──────────────┘
```
...
### 3. Transliteration Agent (`agents/transliteration_agent.py`)

Агент транслитерации символов/текста в удобочитаемую форму.

**Функции:**
- Принимает последовательность распознанных классов YOLO (A, Wau, Iy, …) или текст после LLM.
- Преобразует её по таблице соответствий в латиницу/кириллицу.

**Вход:** Строка с исходным текстом (`raw_text`)  
**Выход:** Строка транслитерации (`transliteration`)
...
print(f"Исходный текст: {final['ocr_results']['raw_text']}")
print(f"Исправленный текст: {final['llm_results']['corrected_text']}")
print(f"Транслитерация: {final.get('transliteration', '')}")
print(f"Лексический анализ: {final['llm_results']['lexical_analysis']}")
```
